...
train_dataset = MovieImageDataset('backdrop_dataset', df_train, transform)
test_dataset = MovieImageDataset('backdrop_dataset', df_test, transform)
...
for epoch in range(epochs):
    ...
    if val_r2 > best_val_r2:
        best_val_r2 = val_r2
        torch.save(model.state_dict(), 'best_backdrop_model.pt')
        print(f"Saved new best model with R² = {val_r2:.4f}")

model.load_state_dict(torch.load('best_backdrop_model.pt'))
model.eval()
